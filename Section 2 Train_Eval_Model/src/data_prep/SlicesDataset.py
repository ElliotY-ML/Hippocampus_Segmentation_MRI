...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        '''
        lists and np.arrays maintain positions.  Dictonaries don't maintain positions
        '''
        #print(f'idx is {idx} Data shape is {self.data[slc[0]].keys()}')
        im_np = self.data[slc[0]]['image'][slc[1],:,:][None,:]
        seg_np = self.data[slc[0]]['seg'][slc[1],:,:][None,:]
        sample['image'] = torch.from_numpy(im_np)
